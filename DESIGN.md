...
- **One text format, wherever a concert is written down.** A song is its four fields separated by
  tabs, a pause is `---`, one entry per line. The Local Store holds exactly that, `Export concert`
  writes exactly that to a `.tsv` file, and `Import concert` reads it back — so a concert can go to
  another machine, or into a spreadsheet, and come home unchanged. `concertText` writes it and
  `rowsOfSaved` reads it, and reading is the looser of the two: any line ending, and a pause is any
  line of three or more dashes.

  **Export and Import, never Download and Upload.** Nothing travels: the app has no server, and the
  file lands on the same machine the Local Store is already on. The `download` attribute on the
  anchor is the browser's own spelling and stays; the words a person reads do not.

  An imported concert is deliberately **not** a load. It did not come from the Local Store, so the
