...
- **Interaction is exercised only where a probe was written.** The probes cover the cue column, the
  ...
  load — and exporting and importing a concert as a file. Theme switching and the volume slider
  are still only ever loaded, never pressed.
- An export can be checked without a file being written: the probe replaces `URL.createObjectURL`
  to keep the Blob and `HTMLAnchorElement.prototype.click` to read the file name off the anchor,
  then reads the Blob back as text. An import is driven by building a `File` in a `DataTransfer`,
...
- **Reading a file is really asynchronous**, and `--virtual-time-budget` races past a fixed wait, so
  a probe that imports must idle for a few steps rather than one. A dialog appearing "one step late"
