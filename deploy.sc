...
//   1. builds main.js via build.sc, which runs the tests first, so a red suite stops the deploy
//   2. on success uploads index.html + main.js + fonts/ over SFTP with lftp
...
//   ./deploy.sc --dry-run    # build + show what WOULD upload, change nothing
//   ./deploy.sc              # build + upload
...
// ---- step 1: build ----
println("deploy: building via build.sc")
val buildRc = new ProcessBuilder("scala-cli", "run", "build.sc").inheritIO().start().waitFor()
